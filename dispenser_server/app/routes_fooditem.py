...
def read_all_food_items_from_inventory():
    inventory_id = request.form.get('inventory_id')
    food_items = Food_Item.query.with_for_update(read=True).filter_by(inventory_id=inventory_id)
...
def read_all_food_items_from_wishlist():
    ...
    food_items = Food_Item.query.with_for_update(read=True).filter_by(wish_list_id=wishlist_id)
    return jsonify([food_item.to_json() for food_item in food_items]), 200


@app.route('/api/food_item/read/<int:food_item_id>', methods=['GET'])
def read_food_item(food_item_id):
    food_item = Food_Item.query.with_for_update(read=True).get(food_item_id)
...
def update_food_item(food_item_id):
    ...
    food_item = Food_Item.query.with_for_update().get(food_item_id)
    if food_item is None:
        abort(404)
