...
class CategoryColumn extends StatefulWidget {
  ...
  CategoryColumn(this.updateParentState, this.repository);

  @override
  State<StatefulWidget> createState() {
    return CategoryColumnState();
  }
}

class CategoryColumnState extends State<CategoryColumn> {
  num quantity = 1;
  int openedTile = -1;
  ...
  void updateRepository() {
    ...
    for (int i = widget.repository.foodItems.length; i < products.length; i++) {
      ObjFoodItem newProduct =
          ObjFoodItem(products[i]['name'], products[i]['quantity'], 1);
